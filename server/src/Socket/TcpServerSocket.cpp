...
namespace ts
{
  namespace common
  {
	...
	mAcceptor.async_accept(newClient->getSocket(),
			       [this, newClient](boost::system::error_code const &ec) {
      ...
      ServerSocket::createEndpointFromAddress(std::string const &host, unsigned short port)
      {
	if (host.empty())
	...
	return {boost::asio::ip::address::from_string(host), port};
      ...
      void ServerSocket::setOnClientAcceptedCallback(OnClientAcceptedFunc onClientAccepted)
      {
