...
#include "Socket/UdpClientSocket.hpp"
  ...
  namespace common
  {
	...
	return mSocket;
      ...
      size_t ClientSocket::sendTo(const std::string &host,
				  ...
				  size_t size)
      {
	boost::asio::ip::udp::endpoint ep(boost::asio::ip::address::from_string(host.c_str()), port);
	...
	mSocket.async_send_to(boost::asio::buffer(data, size), ep,
      ...
      size_t ClientSocket::receiveFrom(const std::string &host, unsigned short port, void *data, size_t size)
      {
	boost::asio::ip::udp::endpoint ep(boost::asio::ip::address::from_string(host.c_str()), port);

	return mSocket.receive_from(boost::asio::buffer(data, size), ep);
      }

      void ClientSocket::asyncReceiveFrom(const std::string &host,
					  unsigned short port,
					  void *data,
					  size_t size,
	...
	mSocket.async_receive_from(boost::asio::buffer(data, size), ep,
				   [=](const boost::system::error_code &ec, size_t ret){
      ...
      void ClientSocket::setOnReceiveFromCallback(OnReceiveFromFunc onReceiveFrom)
      {
	mOnReceiveFrom = onReceiveFrom;
      }
