...
namespace ts
{
  namespace common
  {
	...
	ret = boost::asio::write(mSocket, boost::asio::buffer(data, size), errorCode);
	...
	ret = boost::asio::read(mSocket, boost::asio::buffer(data, size), errorCode);

	return (!errorCode ? ret : 0);
      }

      void ClientSocket::asyncWrite(void const *data, size_t size, OnWriteFunc onWrite)
      {
	if (onWrite != nullptr)
	  mOnWrite = onWrite;

	boost::asio::async_write(mSocket, boost::asio::buffer(data, size),
				 [=](const boost::system::error_code &ec, size_t ret) {
	...
	return mPort;
      ...
      void ClientSocket::asyncConnect(std::string const &host, unsigned short port, OnConnectFunc onConnect)
      {
	...
	startAsyncConnect(host, port);
      ...
      void ClientSocket::startAsyncConnect(std::string const &host, unsigned short port)
      {
	mSocket.async_connect({boost::asio::ip::address::from_string(host), port},
	...
	mIos.run();
      ...
      {
	return ((boost::asio::error::eof == ec) || (boost::asio::error::connection_reset == ec));
      }
